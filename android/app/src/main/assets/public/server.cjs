...
Word to pronounce: "${trimmedText}"`;
    ...
    const durationSeconds = +(rawBuffer.length / (sampleRate * 2)).toFixed(2);
    const safeWord = trimmedText.slice(0, 24).toLowerCase().replace(/[^a-z0-9_-]+/gi, "-").replace(/^-+|-+$/g, "");
    ...
    const message = error?.message || "Failed to generate speech. Please try again.";
...
Always keep German sentences natural, encouraging, and tailored to level ${level}.`;
    ...
    try {
      ...
      parsed = {
        ...
        vocabularyTip: null,
        grammarNote: null,
        suggestedReplies: ["Wie geht es Ihnen?", "K\xF6nnen Sie das wiederholen?"]
      };
    }
...
app.post("/api/studio/generate-script", async (req, res) => {
  try {
    const {
      ...
      // 'video_voiceover' | 'german_dialogue' | 'educational'
